...
<div class="glass p-8 rounded-2xl flex flex-col gap-6 max-w-md w-full mx-auto animate-in fade-in zoom-in duration-300">
  {#if !authState.isSignupRequired}
    <form onsubmit={handleLogin} class="flex flex-col gap-4">
      <h2 class="text-2xl font-bold text-deep-raspberry">Welcome Back</h2>
      <p class="text-sm text-deep-raspberry/70">Enter your username to continue.</p>
      
      <div class="flex flex-col gap-1">
        <label for="username" class="text-sm font-medium text-deep-raspberry">Username</label>
        <input
          type="text"
          id="username"
          bind:value={username}
      ...
      <button
        ...
        class="mt-2 bg-vivid-pink text-white font-bold py-3 rounded-xl skeuo-button disabled:opacity-50 animate-heartbeat"
      >
  ...
  {:else}
    <form onsubmit={handleSignup} class="flex flex-col gap-4">
      ...
      <p class="text-sm text-deep-raspberry/70">We couldn't find your username. Please tell us your display name to create an account.</p>
      
      <div class="flex flex-col gap-1">
        <label for="senderName" class="text-sm font-medium text-deep-raspberry">Your Display Name</label>
        <input
          type="text"
          id="senderName"
